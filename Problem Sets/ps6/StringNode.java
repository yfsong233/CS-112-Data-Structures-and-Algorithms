...
public class StringNode {
    ...
    /**
     * convert() recursively converts a standard Java String object to a linked-list
     * string and returns a reference to the linked-list string
     */
    public static StringNode convert(String s) {
        ...
        StringNode convertRest = convert(s.substring(1));
        return new StringNode(s.charAt(0), convertRest);
    }
    ...
    /*
     * copy() uses an iterative approach to return 
     * a copy of the given linked-list string
     */
    public static StringNode copy(StringNode str) {
        if (str == null) {  // an empty list
            return null;
        }
        StringNode trav = str; // trailing reference for str
        StringNode strCopied = new StringNode(str.ch, str.next);
        StringNode trail = strCopied; // trailing reference for strCopied
        
        if (str.next == null) {  // handling the case of a list with one node (REDUNDANT)
            return strCopied;
        }
        
        while (trav.next != null) {
            trail.next = new StringNode(trav.next.ch, trav.next.next);
            trav = trav.next;
            trail = trail.next;
        }
    ...
    /**
     * insertSorted() uses a recursive approach 
     * to insert character ch in the correct position
     * in a sorted list of characters (i.e., a sorted linked-list string)
     * and returns a reference to the resulting list.
     */
    public static StringNode insertSorted(StringNode str, char ch) {
        if (str == null) {
            return new StringNode(ch, null);  // allow null as imput
        }
        if (str.ch < ch) {
    ...
    /**
     * length() uses an iterative approach 
     * to determines the number of characters in the
     * linked-list string to which str refers
     */
    public static int length(StringNode str) {
        StringNode trav = str;
    ...
    public static StringNode reverse(StringNode str) {
        StringNode trav = str;  // assume valid input of str
    ...
    /**
     * printReverse() uses an iterative approach to write 
     * the reverse of the specified linked-list string to System.out
     * 
     * Two possible implementations:
     * a) reverse the linked list before printing it
     * b) use an array
     */
    public static void printReverse(StringNode str) {
        StringNode head = str;
        ...
        head = reverse(head); // reversal is completed
        trav = head;

        while (trav != null) {
            System.out.print(trav.ch); // print the linked-list in reversal
            trav = trav.next;
        }

        head = reverse(head); // restore the linked-list
    ...
    /**
     * removeFirst() uses a recursive approach to take 
     * the linked-list string specified by str and 
     * remove the first occurrence (if any) of the character ch in that string.
     */
    public static StringNode removeFirst(StringNode str, char ch) { 
        if (str == null) {
            return null;
    ...
    /**
     * toUpperCase() uses a recursive approach to 
     * converts all of the characters in the specified
     * linked-list string to upper case.  Modifies the list itself,
     * rather than creating a new list.
     */
    public static void toUpperCase(StringNode str) {
